...
def custom_lr_scheduler(optimizer, epoch,mode=2):#两种学习策略均可使用，模式一较稳定
    if mode==1:
...
def train(lamada,adaptation_dim,denoise_step=[85,95],kmeans=False,stage=[80,160],scheduler=True,mode=2):
    freeze_support()
    ...
    target_train_dataset = ImagePathDataset(datasets.ImageFolder(root='D:\dataset\SAR\SAMPLE_dataset_public-master\png_images\qpm\\real_elev_17'),transform=test_transform)
...
if __name__ == '__main__':
    for lamada in [0.75]:
        for adaptation_dim in [256]:
            for mode in [1, 2]:
                for denoise_step, stage  in zip([ [75, 85, 95],[70, 90], [75, 85], ], [[80, 160, 220],[80, 160], [80, 160],  ]):
                    result = []
                    file_disk = r'G:\python_code\Scripts\SAMPLE_resnet\Long-CLIP\SAMPLE_resnet_K\result_txt\sence2_lr'
                    ...
                    for i in range(20):
                        current = train(lamada, adaptation_dim, denoise_step=denoise_step, kmeans=False, stage=stage, scheduler=True, mode=mode)
